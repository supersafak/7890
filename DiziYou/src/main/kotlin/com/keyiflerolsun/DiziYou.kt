...
class DiziYou : MainAPI() {
    ...
    private fun Element.toMainPageResult(): SearchResponse? {
        ...
        val score = this.selectFirst("div#imdbp")?.text()?.replace("(", "")?.replace(")", "")
    ...
    override suspend fun load(url: String): LoadResponse? {
        ...
        val rating =
            document.selectFirst("span.dizimeta:contains(IMDB)")?.nextSibling()?.toString()?.trim()
        val actors =
            document.selectFirst("span.dizimeta:contains(Oyuncular)")?.nextSibling()?.toString()
                ?.trim()?.split(", ")?.map { Actor(it) }
        ...
        val episodes = document.select("div.bolumust").mapNotNull {
            ...
            newEpisode(epHref) {
                this.name =
                    it.selectFirst("div.bolumismi")?.text()?.trim()?.replace(Regex("""[()]"""), "")
                        ?.trim() ?: epName
